...
export function BetaPage() {
  ...
  const { get, ready } = usePageSearch();
  const platform = get("platform");
  const navigate = useNavigate();
  useLayoutEffect(() => {
    if (ready && platform === "macos") void navigate({
      to: tw ? "/zh-TW/download/" : "/download/",
      search: previous => ({ ...previous, platform: "macos" }),
      replace: true,
    });
  }, [ready, platform, tw, navigate]);
  usePageMeta();
  useReveal();
  if (ready && platform === "macos") return null;
