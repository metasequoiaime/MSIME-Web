...
export const seoPages: Record<string, { title: string; description: string; noindex?: boolean }> = {
  ...
  "/docs/": { title: "使用文档与安装指南｜水杉输入法", description: "水杉输入法 Windows、macOS、macOS 语音与 Linux 使用指南，选择平台查看安装、配置和日常使用方法。" },
...
export function pageSeo(path: string) {
  ...
  return { ...(page ?? { title: "页面不存在｜水杉输入法", description: "这个地址没有内容，请返回首页或查看使用文档。", noindex: true }), path: normalized, canonical: page ? `${SITE_ORIGIN}${normalized}` : undefined };
}
export function structuredData(path: string) {
  ...
  if (page.path === "/") graph.push({ "@type": "SoftwareApplication", "@id": `${SITE_ORIGIN}/#software`, name: SITE_NAME, alternateName: ["MSIME", "MetasequoiaIME"], applicationCategory: "UtilitiesApplication", operatingSystem: ["Windows", "macOS", "Linux"], url: `${SITE_ORIGIN}/`, downloadUrl: `${SITE_ORIGIN}/download/`, license: "https://github.com/metasequoiaime/MSIME-Web/blob/main/LICENSE", publisher: { "@id": organization["@id"] } });
  if (page.path !== "/") {
